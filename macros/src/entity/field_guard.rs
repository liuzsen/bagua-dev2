...
impl Entity {
    ...
    fn expand_subsets(&self) -> syn::Result<TokenStream2> {
        ...
        for subset in &self.subsets {
            ...
            let subset_fields: Fields =
                subset.fields.iter().map(|field| field.to_filed()).collect();
    ...
    fn expand_model(&self) -> syn::Result<TokenStream2> {
        ...
        let output = quote! {
            ...
            impl From<#model_ident> for #entity_ident {
    ...
    fn expand_entity(&self) -> syn::Result<TokenStream2> {
        let fields: Punctuated<Field, Token![,]> = self
            .all_fields
            .iter()
            .filter_map(|field| field.to_guarded_field())
            .collect();
        let entity_ident = &self.name;
        let attrs = &self.attrs;
    ...
    fn impl_unloaded(&self) -> TokenStream2 {
        let name = &self.name;
        let fields = self.all_fields.iter().map(|f| {
        ...
        quote! {
            impl ::bagua::entity::field::Unloaded for #name {
                fn unloaded() -> Self {
                    Self {
                        #(#fields)*
                    }
...
impl EntityField {
    ...
    fn to_guarded_field(&self) -> Option<Field> {
        let mut field = self.inner.clone();
